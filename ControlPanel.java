       ...
       public void actionPerformed(ActionEvent event)
        {
            ...
            //Changes the color of the dot to the choice of the user
            ...
            //Starts the timer again if 'Start' button is clicked
            if (action == start)
            	drawing.resume();
            
            //Stops the timer if 'Stop' button is clicked
            if (action == stop)
            	drawing.suspend();
            
            //Reverses the horizontal direction if the 'Reverse X' button
            //is clicked and starts it again
            if (action == reverseX)
            {
            	drawing.reverseX();
            	drawing.resume();
            }
            //Reverses the vertical direction if the 'Reverse Y' button
            //is clicked and starts it again
            if (action == reverseY)
            {
            	drawing.reverseY();
   ...
   private class SliderListener implements ChangeListener
    {
        public void stateChanged(ChangeEvent event)
         {
        	drawing.setDelay(slider1.getValue()); //sets delay to current value of the slider
         }
